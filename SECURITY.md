...
This repository is the **Java server SDK** — code that runs inside our customers' own JVMs,
holds a genuine secret (the `ffs_` server key), receives the customer's targeting rules, parses
those bytes with its own bounded JSON parser, and is handed the customer's context identifiers
on every evaluation. Findings we especially want to hear about:

- **The server key reaching anywhere but the `Authorization` header.** A log line, an exception
  message, a `toString`, a cache file — the only loggable form is the `ffs_<env>_` prefix plus
  six characters. Anything more is a finding.
- **Customer data leaving in-process evaluation.** Context keys and tags are the customer's data
  (Founding §7.3) — never logged, persisted or transmitted. Anything that widens that is serious.
- **Cache-file exposure.** The opt-in cache holds only the last verified ruleset envelope's raw
  bytes, written atomically. Anything that stores more, or somewhere else, is a finding.
- **Parser bounds.** The in-repo JSON parser caps nesting depth and input size, reports malformed
  UTF-8 rather than replacing it, and rejects trailing garbage. A payload that gets past those
  bounds, or exhausts memory or CPU inside them, is a finding.
- **Host-process impact.** After construction the SDK must never throw to the caller, never
  leave a non-daemon thread keeping the JVM alive, and never block an evaluation path (§8.1).
- **Ruleset or transport integrity** — anything letting an attacker feed the SDK a ruleset it
  should not accept, including a way around the Ed25519 signature verification.

One note specific to this repo:

- **We cannot recall a shipped SDK.** A vulnerable version lives in customers' deployments until
  they upgrade and redeploy, on their schedule. That makes findings here longer-lived than
  anywhere in the control plane, and worth reporting even when they look minor.

Other components live in their own repositories, each with this policy: `FortressFlag_Standards`
(the contract), the sibling SDKs (`FortressFlag_SDK_ios`, `FortressFlag_SDK_android`,
`FortressFlag_SDK_web`, `FortressFlag_SDK_go`, `FortressFlag_SDK_node`,
`FortressFlag_SDK_python`), `FortressFlag_Backend` (control plane), `FortressFlag_Frontend`
