...
customers' processes, holds a genuine secret, receives targeting rules, parses hostile
bytes with its own parser, and is handed context identifiers. Tick every box that applies —
or tick the last one. A block with nothing ticked is an unfinished PR.
-->
...
- [ ] **Parser surface** — touches `com.fortressflag.server.json` (bounds, accepted grammar, error paths — CLAUDE.md §3)
- [ ] **Host-process impact** — throws reaching the caller, non-daemon threads, blocking work on evaluation paths (Founding §8.1)
- [ ] **Public API** — the public types of `com.fortressflag.server`; backward compatibility is sacred (Founding §8.3)
...
<!-- The local gate is `./gradlew build`. What did you verify beyond it, especially
against a live backend, which CI cannot do? -->
